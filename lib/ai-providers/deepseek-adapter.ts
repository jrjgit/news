...
export interface DeepSeekAdapterConfig extends Record<string, unknown> {
  apiKey?: string
  model?: string
  baseUrl?: string
}

export class DeepSeekAdapter extends BaseAIAdapter<DeepSeekAdapterConfig> {
  ...
  constructor(config: DeepSeekAdapterConfig) {
    ...
    if (!config.apiKey) {
      console.warn('DeepSeek API密钥未配置，AI功能将不可用')
      return
    }
